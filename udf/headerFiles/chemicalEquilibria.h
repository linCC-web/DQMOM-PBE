    ...
    int* interruptFlag)
{
    ...
    int j;
    int n = N_COMPS, info, validFlag = 0;
    int ipiv[N_COMPS];
    double error;
    for (i = 0; i < MAX_ITER; i++)
    {
        ...
        if (info == 0)
        {
            ...
            for (j = 0; j < N_COMPS; j++)
            {
                ...
                else if (isinf(negf[j]) && validFlag == 1)
                {
                    validFlag = 2;
                }
            }
...
void solve_quadratic_equation(double a, double b, double c, double* pH) {
    ...
    if (discriminant < 0) {
        ...
        if (x1 > 1e-7) {
            ...
            *pH = -pKw + log10(x2);
            ...
            printf("The equation has valid pH roots.\n");
        }
    }
}